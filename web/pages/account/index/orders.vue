      ...
      <div class="wait-payment blocks">
        <div v-for="(o, index) in listData" :key="index" class="blocks-mark">
          ...
          <div v-if="o.orderStatus == 40" class="list-footer">
            ...
            <a v-if="o.isComment">
              <button @click="goToComment(o.id)"> {{ $t(`${lang}.checkEvaluate`) }} </button>
            </a>
            <a v-else>
              <button @click="goToComment(o.id)"> {{ $t(`${lang}.toEvaluate`) }} </button>
            </a>
            <!-- <div
              class="fakeA"
              @click="
          ...
          <!--待评论-->
          <div v-if="o.orderStatus == 5" class="list-footer" style="border: 1px solid red">
            <nuxt-link :to="`/account/order-details?orderId=${o.id}`"
              ><button>
                {{ $t(`${lang}.orderDetail`) }}
              </button>
