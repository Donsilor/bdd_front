...
export default {
    ...
    localAreaSetting ({ $axios, state, getters, commit, dispatch }) {
        ...
        this.$axios({
            ...
            const data = res.data
            // console.log("rrrrrr",data.area_id)
            // console.log("Sdsdsd",window.location.host) 
            let headerHost = window.document.location.host
            //生产环境  window.document.location.host
            let host = '';
            if(parseInt(data.area_id) === 1){ //PC大陆
                // if (!(/cn-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://cn-bdd.bddco.cn'
                // }
                if (!(/www\.bddco\.cn/).test(headerHost)) {
                    //生产环境
                    host = 'https://www.bddco.cn'
    
                }
                
            } else if(parseInt(data.area_id) === 2 || parseInt(data.area_id) === 3 ){ //PC香港
                // if (!(/www-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://www-bdd.bddco.cn'
                // } 
                if (!(/www\.bddco\.com/).test(headerHost)) { 
                    //生产环境
                    host = 'https://www.bddco.com'
                }
                
            } else if(parseInt(data.area_id) === 4){ //PC台湾
                // if (!(/tw-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://tw-bdd.bddco.cn'
                // }
                if (!(/tw\.bddco\.com/).test(headerHost)) { 
                    //生产环境
                    host = 'https://tw.bddco.com'
                }
                
            } else if(parseInt(data.area_id) === 99){ //PC美国
                // if (!(/us-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://us-bdd.bddco.cn'
                // }
                if (!(/us\.bddco\.com/).test(headerHost)) {  
                    //生产环境
                    host = 'https://us.bddco.com'
                } 
            } else {
                // if (!(/www-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
    ...
    */
    // 加入浏览记录 
    addBrowseRecords({ $axios, state, getters, commit, dispatch }, records = []) {
        console.log("加入浏览记录",records)
        let data = null
        ...
        //     // 已登录的操作
        //     // console.log('已登录的操作')
        //     request = dispatch('addOnlineBrowseRecords', data)
        // } else {
            // 未登录的操作
            request = dispatch('addLocalBrowseRecords', data)
        // }
        request
            .then(data => {
                // 加入成功后，重新获取购物车数据
                dispatch('getBrowseRecords')
                return Promise.resolve(data)
            })
            .catch(err => {
                return Promise.reject(err)
    ...
    // 获取本地浏览记录数据
    getLocalBrowseRecords ({ $axios, state, getters, commit, dispatch }) {
        // console.log('获取本地浏览记录数据=====>',res)
        return JSON.parse(localStorage.getItem(RECORDS) || '[]')
    },
    // 加入到并获取线上浏览记录
    addOnlineBrowseRecords ({ $axios, state, getters, commit, dispatch }, records = []) {
        // console.log('addOnlineCart=====>',goods)
        // const time = getTimestampUuid()
        ...
        return this.$axios({
            ...
            .then(data => {
                ...
                console.log("添加本地浏览记录", data.data)
                // 重新请求本地浏览记录（和本地浏览记录列表）
                return data.data
            })
            .catch(err => {
                return Promise.reject(err)
    ...
    // 加入到本地浏览记录
    addLocalBrowseRecords ({ $axios, state, getters, commit, dispatch }, records = []) {
        // console.log("加入到本地浏览记录", records)
        const time = getTimestampUuid()
        let record ={}
        records.map(items => {
            record = items
        ...
        const addInfo = {
            ...
            data: {
                ...
                goodsImages: record.goodsImages,
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let notes = await dispatch('getLocalBrowseRecords')
                notes = notes.concat(addInfo)
                // if (cart.length > 30) {
                //     return reject(new Error(lang.cartIsFull))
                // }
                // console.log("notes",addInfo)
                const result = []
                let tmp = [];
                for(var k=(notes.length-1); k>=0; k--){
                    // console.log("notes4444444444",notes[k],notes.length,k)
                    let key = notes[k].data.id + '-' +notes[k].data.categoryId
                ...
                // 本地加入本地浏览记录
                localStorage.setItem(RECORDS, JSON.stringify(result.reverse()))
                return resolve()
            } catch (e) {
                // console.log("eeeeee", e)
                return reject(e)
    ...
    // 获取浏览记录数据
    getBrowseRecords ({ $axios, state, getters, commit, dispatch }) {
        let request = null
        // if (getters.hadLogin) {
        //     // 已登录的操作
        //     // console.log('已登录的操作')
        //     request = dispatch('getOnlineBrowseRecords')
        // } else {
            // 未登录的操作
            // console.log('未登录的操作')
            request = dispatch('getLocalBrowseRecords')
            // return Promise.reject(new Error('请登录！'))
        // }
        // dispatch('getCartAmount')
        request
            .then(data => {
                // console.log("本地浏览记录数据",data)
                commit('setBrowseRecords', data)
                return Promise.resolve(data)
            })
            .catch(err => {
                return Promise.reject(err)
