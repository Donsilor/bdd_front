    ...
    <div class="paylist" v-show="paylist">
      ...
      <div class="transfer" v-show="transfer">
        <div class="transfer-header">
          <i class="icon iconfont iconfanhuiicon-" @click="closed">{{ lang2.back }}</i>
          <h4>{{ lang2.pleaseSelectAccount }}</h4>
        </div>
        <Upload @cancel="closed" :ultimatelyPay="ultimatelyPay" :transferOrderId="transferOrderId"></Upload>
      </div>
    </div>
</template>
    ...
    goAddress() {
      ...
      if(!this.addressLength){
        ...
        var timeOut = setTimeout(function() {
          _this.$children[7].editAddress(null)
          ...
          if(data.length){
            this.$axios({
              ...
              .then(res => {
                ...
                }else if(res.order_sn){
                  this.$nuxt.$loading.finish()
                  this.paylist = false
                  this.transfer = true
                  this.transferOrderId = res.order_sn
                } else if (!res.config){
                  this.isPay = false
                  this.$router.replace({
