<template>
  <div class="container">
    ...
    <div class="box">
      ...
      <div class="card-box">
        <!-- 购物卡列表 -->
        <div class="list" v-for="(item, index) in cardList" :key="index" ref="list">
          <div class="card-info">
    ...
    <div class="popup" v-if="ifShowPop" :class="verifyStatus == 2 || verifyStatus == 3? 'invalid' : ''" @click="closePop()">
      ...
      <div class="pop-box" v-if="verifyStatus == 3">
        <div class="icon"></div>
        <div class="status">{{ !cardList[nowIndex].noSupport ? lang.noSupport : lang.verifyInvalid }}</div>
        <div class="btn">{{ lang.anewAdd }}</div>
      </div>
...
<script>
  export default{
    ...
    methods:{
      ...
      // 验证
      verification(k){
        // console.log("dddddd",this.verifyStatus)
        ...
        if(this.cardList[k].account == '' || this.cardList[k].conversionNum == ''){
          ...
          if(!ifRepetition){
            ...
            .then(res => {
              ...
              that.areaUse = res.areaUse
              that.cardList[k].areaTips = res.areaTips
              that.cardList[k].areaUse = res.areaUse
              ...
              if(that.areaUse == false){
                ...
                that.verifyStatus = 2;
				        that.cardList[k].type = 2;
                that.$toast.show(this.lang.msg13);
              }else if(that.areaUse == 0){
                that.verifyStatus = 3;
                that.$errorMessage(res.areaTips);
              }
            })
            .catch(err => {
      ...
      // 选择单个购物卡
      chooseList(k){
        var that=this, flag=true, flag2=true, flag5=true;
          if(this.cardList[k].ifChoose == false){
            ...
            if(!this.cardList[k].areaUse){
               if(this.cardList[k].ifAllowedSite){
                  ...
                  this.$toast.show(that.cardList[k].areaTips);
                }
            }else{
      ...
      allChoose(){
        ...
        if(this.ifChooseAll == false){
          for(var i of this.cardList){
            ...
            if(i.type == 1 && i.balance !== 0 && i.ifAllowedSite){
               i.ifChoose = true;
            }else {
      ...
      removeCard(k){
          ...
          this.cardList[k].ifAllowedSite = false
          this.cardList[k].ifShowRemove = false;
