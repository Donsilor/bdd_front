...
export default {
    ...
    localAreaSetting ({ $axios, state, getters, commit, dispatch }) {
        ...
        this.$axios({
            ...
            // console.log("Sdsdsd",data.area_id) 
            let headerHost =  window.location.host
            //生产环境  window.document.location.host
            let host = '';
            if(parseInt(data.area_id)  === 1){ //PC大陆
                // if (!(/wap-cn-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://wap-cn-bdd.bddco.cn'
                // }
                if (!(/wap\.bddco\.cn/).test(headerHost)) {
                    ////正式环境
                    host = 'https://wap.bddco.cn'
                }
                
            } else if(parseInt(data.area_id) === 2 || parseInt(data.area_id) === 3){ //PC香港
                // if (!(/wap-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://wap-bdd.bddco.cn'
                // } 
                if (!(/wap\.bddco\.com/).test(headerHost)) {
                    //正式环境
                    host = 'https://wap.bddco.com'
                } 
                
            } else if(parseInt(data.area_id) === 4){ //PC台湾
                // if (!(/wap-tw-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://wap-tw-bdd.bddco.cn'
                // }
                if (!(/wap-tw\.bddco\.com/).test(headerHost)) {
                    //正式环境
                    host = 'https://wap-tw.bddco.com'
                }
                
            } else if(parseInt(data.area_id) === 99){ //PC美国
                // if (!(/wap-us-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
                //     host = 'https://wap-us-bdd.bddco.cn'
                // }
                if (!(/wap-us\.bddco\.com/).test(headerHost)) {
                    //正式环境
                    host = 'https://wap-us.bddco.com'
                }
            } else {
                // if (!(/wap-bdd\.bddco\.cn/).test(headerHost)) {
                //     //测试环境 
    ...
    */
    ...
    getLocalBrowseRecords ({ $axios, state, getters, commit, dispatch }) {
        // console.log('获取本地浏览记录数据=====>',localStorage.getItem(RECORDS))
        return JSON.parse(localStorage.getItem(RECORDS) || '[]')
    },

    // 加入到线上浏览记录
    addOnlineBrowseRecords ({ $axios, state, getters, commit, dispatch }, records = []) {
        // console.log('addOnlineBrowseRecords=====>',records)
        // const time = getTimestampUuid()
